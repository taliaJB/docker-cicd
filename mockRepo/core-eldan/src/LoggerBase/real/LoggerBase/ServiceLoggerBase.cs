...
namespace Eldan.LoggerBase
{
    ...
    public abstract class ServiceLoggerBase : LoggerBase
    {
        enum enmMaskReturnValueType
        {
            None,
            All,
            ByMethod,
        ...
        const string MASKED_VALUE = "(Value Masked)";

        List<string> m_MaskMethodsNames;
        List<string> m_MaskAllMethodsNamesExcept;
        enmMaskReturnValueType m_MaskReturnValue;
        List<string> m_MaskParamsNames;
        IServiceLoggerEvents m_ServiceLoggerEvents;
        List<Exception> m_ExceptionsToAvoid;
        bool m_AddEldanIDInErrorMessage;
        int? m_MaxChars;

        Dictionary<string, string> m_configuration;

        protected ServiceLoggerBase(Dictionary<string, string> configuration) : this(configuration, "EldanService")
        { }

        protected ServiceLoggerBase(Dictionary<string, string> configuration, string serviceName) : base(configuration)
        {
            m_configuration = configuration;
            m_ServiceName = serviceName;
            m_ThrowExceptionWhenError = true;
            m_LoggerFilePathKey = LoggerBase.DEFAULT_FILE_PATH_KEY;
            ...
            int MaxChars;
            if (int.TryParse(GetConfigVal("serviceLoggerMaxChars"), out MaxChars))
                if (MaxChars < 0)
                    m_MaxChars = null;
                else
                    m_MaxChars = MaxChars;
            ...
            if (!bool.TryParse(GetConfigVal("serviceLoggerAddEldanIDInErrorMessage"), out m_AddEldanIDInErrorMessage))
                m_AddEldanIDInErrorMessage = false;

            string Methods = GetConfigVal("serviceLoggerMaskMethodsReturnValue");
            ...
            string MaskParams = GetConfigVal("serviceLoggerMaskParams");
        ...
        public T CreateInstance<T>() where T : new()
        {
            ...
            try
            {
                if (m_LogCreateInstance)
                    Write(string.Format("{0}.{1}()- started", m_ServiceName, m_ServiceName),
                        enmLogType.Information, m_LoggerFilePathKey);
                Instance = new T();

                if (Instance is IServiceLoggerBL ServiceLoggerBL)
                    ServiceLoggerBL.ServiceLogger = this;
                else
                    Write("ServiceLoggerBase.CreateInstance - BL dose not implement interface: 'IServiceLoggerBL'", enmLogType.Warning, m_LoggerFilePathKey);
            }
            catch (Exception ex)
            {
        ...
        private void LogCompleteAction(MethodInfo Method, params object[] Params)
        {
            ...
            if (LogComplete)
                Write(string.Format("{0}.{1} - completed)", m_ServiceName, MethodName),
                    enmLogType.Information, m_LoggerFilePathKey);

            if (m_ServiceLoggerEvents != null)
            {
                if (Method == null)
                    m_ServiceLoggerEvents.logCreateBL(m_ServiceName, LoggerSessionID);
                else
                    m_ServiceLoggerEvents.LogMethodEnd(m_ServiceName, Method, Params, null, LoggerSessionID);
            }
        }
        ...
        private void LogCompleteFunc(MethodInfo Method, object RetrunValue, params object[] Params)
        {
            ...
            Write(string.Format("{0}.{1} - completed), returns: {2}", m_ServiceName, Method.Name, Value),
                enmLogType.Information, m_LoggerFilePathKey);

            m_ServiceLoggerEvents?.LogMethodEnd(m_ServiceName, Method, Params, RetrunValue, LoggerSessionID);
        }
        ...
        private void ThrowServiceException(Exception ex, MethodInfo Method, params object[] Params)
        {
            ...
            Write(string.Format("{0}.{1}", m_ServiceName, (Method == null) ? "<Constructor>" : Method.Name),
            ...
            if (m_ServiceLoggerEvents != null)
                m_ServiceLoggerEvents.LogMethodException(m_ServiceName,
                                                         Method,
                                                         Params,
                                                         m_AddEldanIDInErrorMessage ? exEldan.GetExtendedException() : exEldan.GetExtendedException(false), LoggerSessionID);
        ...
        private void LogMethod(MethodInfo Method, params object[] Params)
        {
            ...
            Write(string.Format("{0}.{1}({2}) - started)", m_ServiceName, Method.Name, ParamsText),
                enmLogType.Information, m_LoggerFilePathKey);

            m_ServiceLoggerEvents?.LogMethodStart(m_ServiceName, Method, Params, LoggerSessionID);
        }
        ...
        //public void Write(string Message, clsLoggerBase.enmLogType LogType)
        //{
        //    new clsLoggerBase(m_configuration).Write(Message, LogType);
        //}

        //public void Write(string Message, clsLoggerBase.enmLogType LogType, string FilePathKey)
        //{
        //    new clsLoggerBase(m_configuration).Write(Message, LogType, FilePathKey);
        //}

        //public new Exception Write(string Message, Exception ex)
        //{
        //    EldanError exEldan = (EldanError)ex;

        //    Write(Message, GetExceptionToWrite(exEldan));
        //    return GetExceptionToThrow(exEldan);
        //}

        //public new Exception Write(string Message, Exception ex, string FilePathKey)
        //{
        //    EldanError exEldan = new EldanError(ex);

        //    Write(Message, GetExceptionToWrite(exEldan), FilePathKey);
        //    return GetExceptionToThrow(exEldan);
        //}

        //public new Exception Write(Exception ex)
        //{
        //    EldanError exEldan = new EldanError(ex);

        //    Write(GetExceptionToWrite(exEldan));
        //    return GetExceptionToThrow(exEldan);
        //}

        //public new Exception Write(Exception ex, string FilePathKey)
        //{
        //    EldanError exEldan = new EldanError(ex);

        //    Write(GetExceptionToWrite(exEldan), FilePathKey);
        //    return GetExceptionToThrow(exEldan);
        //}
        ...
        {
            Write(Message, ex, m_LoggerFilePathKey);
        }

        public override void Write(Exception ex)
        {
            Write(ex, m_LoggerFilePathKey);
        }

        public override void Write(string Message, enmLogType LogType)
        {
            Write(Message, LogType, m_LoggerFilePathKey);
        }
